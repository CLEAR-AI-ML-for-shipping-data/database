...
from db_schema import AIS_Data, ClearAIS_DB, Nav_Status, Ships
...
def dateparse(datetime_str: str):
...
def read_and_transform_csv_chunk(file_path, chunk_size=10000):
    ...
    for chunk in pd.read_csv(
        ...
        parse_dates=["Base station time stamp"],
        date_format=dateparse,
    ):
        # Ensure the columns are named correctly
        chunk.drop(
            chunk.columns[chunk.columns.str.contains("unnamed", case=False)],
            axis=1,
            inplace=True,
        )
        ...
        ships_data = ships_data.drop_duplicates(subset=["mmsi"]).to_dict(
            orient="records"
        )
...
def bulk_insert_data_chunked(file_path, database_url, chunk_size=10000):
    ...
    for ships_data, ais_data, nav_status_set in read_and_transform_csv_chunk(
        ...
        with bulk_inserter.Session() as session:
            mmsi2ship_id_map = {
                ship.mmsi: ship.ship_id for ship in session.query(Ships).all()
            }
            ...
            for ais_record in ais_data:
                ais_record["navigational_status"] = nav_status_id_map.get(
                    ais_record["navigational_status_text"], 0
                )
...
def process_files_in_parallel(csv_files, database_url, chunk_size=50000):
    ...
    with Pool(num_cores) as pool:
        ...
        pool.starmap(
            bulk_insert_data_chunked,
            [(file_path, database_url, chunk_size) for file_path in csv_files],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datapath", type=str, default=folder_path, help="csv files directory"
    )
    parser.add_argument(
    ...
    if os.path.exists(path):
        ...
        if os.path.isfile(path):
            ...
            process_files_in_parallel(
                csv_files=csv_files, database_url=database_url, chunk_size=50000
            )
